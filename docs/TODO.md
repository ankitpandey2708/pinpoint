...
```
pinpoint C:\path\to\repo --url https://example.com
```
...
- HTTP/WebSocket reverse proxy with overlay injection — `src/preview/proxy.ts`
  (`proxyRequest`, `handleUpgrade`) and `injectFullOverlay` in
  `src/preview/inject.ts`. Framework previews already run through this path
  against a loopback dev server.
- A `PreviewSession` carries a `proxyUrl` target — `src/preview/registry.ts`.

**What implementing it needs:**

1. Re-add the `--url <public-url>` CLI option and thread it through
   `startReview` in `src/app/cli.ts` (previously stored as `project.publicUrl`).
...
3. An **approximate source resolver**: map a submitted client annotation
   ...
   real work — the submit path in `src/feedback/routes.ts` currently requires a
