...
import { runGit, runGitOrThrow } from '../../platform/git';
...
export async function createReviewBranch(
  ...
  await runGitOrThrow(repoRoot, ['checkout', '-b', branch, baseCommit]);
}
...
export async function commitAll(
  ...
  await runGitOrThrow(repoRoot, ['add', '-A']);
  await runGitOrThrow(repoRoot, [
    '-c',
  ...
  const sha = await runGitOrThrow(repoRoot, ['rev-parse', 'HEAD']);
